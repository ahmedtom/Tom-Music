...
async def join_chat(c: Client, m: Message):
    ...
    try:
        ...
        return await user.send_message(chat_id, "✅ دخلت اهو يعم متزعقش")
    except UserAlreadyParticipant:
        return await user.send_message(chat_id, "✅ موجود اهو ياعم")
...
async def start_group_call(c: Client, m: Message):
    chat_id = m.chat.id
    msg = await c.send_message(chat_id, "`❤️‍🔥يَاެݪله هفتح مَكَالمة...`")
    try:
        ...
        await msg.edit_text("🦴 تَم فَتحت مَكَاެݪمَة اطلعو !")
...
async def stop_group_call(c: Client, m: Message):
    chat_id = m.chat.id
    msg = await c.send_message(chat_id, "`🦎 يَاެݪݪهَ هقفل المَكَاެݪمَةَ...`")
    try:
        if not (
            group_call := (
            ...
            await msg.edit_text("🦎 قفلتها منزمان")
            return
        await user.send(
            DiscardGroupCall(
                call=group_call
        ...
        await msg.edit_text("🦴 تَم اެݪانِهاء  ")
